...
public class SolrSimilarPatientsFinder implements SimilarPatientsFinder, Initializable
{
    ...
    private List<PatientSimilarityView> find(Patient referencePatient, boolean prototypes)
    {
        this.logger.debug("Searching for patients similar to [{}] using access level {}",
            referencePatient.getDocumentReference(), this.accessLevelThreshold.getName());
    ...
    private SolrQuery generateQuery(Patient referencePatient, boolean prototypes)
    {
        ...
        this.logger.debug("SOLRQUERY generated for matching patient [{}]: {}", referencePatient.getDocumentReference(),
            query.toString());
        return query;
    }
