 ...
 * @since 1.0M10
 */
public class DefaultFeatureSimilarityView implements FeatureSimilarityView
{
     ...
     */
    public DefaultFeatureSimilarityView(Feature match, Feature reference)
    {
        this.match = match;
    ...
    {
        return this.match != null ? this.match.isPresent() : true;
    }
    ...
    private void matchMetadata()
    {
        ...
        if (this.match != null && this.match.getMetadata() != null) {
            for (Map.Entry<String, ? extends FeatureMetadatum> entry : this.match.getMetadata().entrySet()) {
                this.matchedMetadata.put(entry.getKey(), new DefaultFeatureMetadatumSimilarityView(entry.getValue(),
                    getMetadatumIfExists(entry.getKey(), this.reference)));
            }
        }
            ...
            for (Map.Entry<String, ? extends FeatureMetadatum> entry : this.reference.getMetadata().entrySet()) {
                if (!this.matchedMetadata.containsKey(entry.getKey())) {
                    this.matchedMetadata.put(entry.getKey(),
                        new DefaultFeatureMetadatumSimilarityView(null, entry.getValue()));
                }
            }
        }
